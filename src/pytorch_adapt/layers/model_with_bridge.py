...
class ModelWithBridgeC(torch.nn.Module):
    """
    ...
    def forward(
        self, x: torch.Tensor, return_bridge: bool = False, return_all_features: bool = False
        ...
        """
        y = self.model(x, return_all_features=return_all_features)
        z = self.bridge(x, return_all_features=return_all_features)
        if return_all_features:
            output = [_y - _z for _y, _z in zip(y, z)]
        else:
            output = y - z
        if return_bridge:
            return output, z
