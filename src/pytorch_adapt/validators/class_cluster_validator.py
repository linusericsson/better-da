...
class ClassClusterValidator(BaseValidator):
    def __init__(
        ...
        **kwargs
        ...
        self.score_fn_types = score_fn_types
        self.score_fn_names = score_fn_names
    ...
    def compute_score(self, target_train, src_train=None):
        ...
        return {n: self.get_clustering_performance(
            feats,
            labels,
            num_classes,
            f,
        ...
        ) for i, (f, t, n) in enumerate(zip(self.score_fns, self.score_fn_types, self.score_fn_names))}


    # copied from https://github.com/lr94/abas/blob/master/model_selection.py
    def get_centroids(self, data, labels, num_classes):
    ...
    def get_clustering_performance(
        ...
        if save:
            ...
            if centroid_init == "label_centers":
                centroids = self.get_centroids(feats, labels, num_classes)
            ...
            self.feats, self.clabels, self.labels = feats, clabels, labels

        if score_fn_type == "labels":
            score = float(score_fn(self.labels, self.clabels))
        elif score_fn_type == "features":
            score = float(score_fn(self.feats, self.clabels))
