...
class GoukValidator(BaseValidator):
    """
    ...
    def compute_distance(self, src_val):
        ...
        d = []
        for true_label in tqdm(range(self.num_classes)): # loop over true label values, y
            l1_norm = torch.linalg.vector_norm(src_confmat[true_label] - src_confmat, ord=torch.inf, dim=1)
            l1_norm[true_label] = torch.inf
            d.append(l1_norm.min())
...
class GoukV3Validator(BaseValidator):
    """
    The sum of [ErrorValidator][pytorch_adapt.validators.ErrorValidator]
    and [L1Validator][pytorch_adapt.validators.L1Validator]
    src_val["labels"], src_val["preds"], target_train["preds"]
    ...
    def compute_distance(self, src_val):
        ...
        for true_label in tqdm(range(self.num_classes)): # loop over true label values, y
            l1_norm = torch.linalg.vector_norm(src_confmat[true_label] - src_confmat, ord=1, dim=1)
            l1_norm[true_label] = torch.inf
            d.append(l1_norm.min() * src_val["labels"].eq(true_label).sum() / denom)
