...
class SYNTHIA(Cityscapes):
    def __init__(self,
        ...
        if self.split == "trainval":
            train_item_list_filepath = os.path.join(self.list_path, "train.txt")
            val_item_list_filepath = os.path.join(self.list_path, "val.txt")
            self.items = [id for id in open(train_item_list_filepath)]
            self.items.extend([id for id in open(val_item_list_filepath)])
        else:
