...
class Office31(BaseDownloadableDataset):
    ...
    def set_paths_and_labels(self, root):
        labels_file = os.path.join(root, "office31", f"{self.domain}_{self.split}.txt")
        ...
        check_length(
            self,
            {
                "amazon": {"train": 1690, "val": 563, "trainval": 2253, "test": 564}[self.split],
                "dslr": {"train": 298, "val": 99, "trainval": 397, "test": 101}[self.split],
                "webcam": {"train": 477, "val": 159, "trainval": 636, "test": 159}[self.split],
            }[self.domain],
        )
