...
from .segmentation import SLossHook
...
class AlignerPlusRHook(BaseWrapperHook):
    """
    Computes an alignment loss plus a regression loss,
    and then optimizes the models.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        [pre, post] = c_f.many_default([pre, post], [[], []])
        aligner_hook = ManyAlignerHook(
            loss_fn=loss_fn, aligner_hook=aligner_hook
        )
        hook = ChainHook(*pre, aligner_hook, RLossHook(), *post)
        hook = OptimizerHook(hook, opts, weighter, reducer)
        s_hook = SummaryHook({"total_loss": hook})
        self.hook = ChainHook(hook, s_hook)


class AlignerPlusSHook(BaseWrapperHook):
    """
    Computes an alignment loss plus a segmentation loss,
    and then optimizes the models.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        hook = ChainHook(*pre, aligner_hook, SLossHook(), *post)
        hook = OptimizerHook(hook, opts, weighter, reducer)
        s_hook = SummaryHook({"total_loss": hook})
        self.hook = ChainHook(hook, s_hook)
